package www.cmj.goods.book.dao;
...
public class BookDao {
	...
	public void edit(Book book) throws SQLException {
		String sql = "update t_book set bname=?,author=?,price=?,currPrice=?," +
				"discount=?,press=?,publishtime=?,total=?,wordNum=?,cid=? where bid=?";
		Object[] params = {book.getBname(),book.getAuthor(),
				book.getPrice(),book.getCurrPrice(),book.getDiscount(),
				book.getPress(),book.getPublishtime(),book.getTotal(),book.getWordNum(),
	 ...
	 * 按bid查询
	 */
	public PageBean<Book> findbook(int pc) throws SQLException {
	...
	private PageBean<Book> findByCriteria(List<Expression> exprList, int pc) throws SQLException {
		 ...
		 * 4. 得到beanList，即当前页记录
		 */
		sql = "select * from t_book" + whereSql + " order by bid limit ?,?";
	...
	public void add(Book book) throws SQLException {
		String sql = "insert into t_book(bid,bname,author,price,currPrice," +
				"discount,press,publishtime,wordNum,cid,image_w,image_b)" +
				" values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
